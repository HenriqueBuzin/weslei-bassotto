...
test("approved subscriber completes the initial questionnaire", async ({ page }) => {
  ...
  await page.route("**/api/v1/me", (route) => route.fulfill({ json: { id: "u1", email: "user@example.com", roles: ["user"] } }));
  await page.route("**/api/v1/payments/pay-1/status**", (route) => route.fulfill({ json: { id: "pay-1", status: "approved" } }));
  await page.route("**/api/v1/consultancy/submissions", async (route) => {
    ...
    await route.fulfill({ status: 201, json: {
      id: "s1", customer: { name: "Aluno", email: "user@example.com", phone: "(54) 99999-0000" },
      plan: { name: "Plano Trimestral", start_date: "2026-01-01", end_date: "2026-04-01" }, status: "active",
      answers: [], renewals: [], answer_revisions: [], renewal_count: 0, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z",
    } });
  });

  const target = "/questionario?plano=trimestral&payment_id=pay-1&payment_token=claim-secret";
  await page.goto(`/login?returnTo=${encodeURIComponent(target)}`, { waitUntil: "domcontentloaded" });
...
test("pending payment never exposes questionnaire fields", async ({ page }) => {
  ...
  await page.route("**/api/v1/consultancy/questions", (route) => route.fulfill({ json: [] }));
  await page.route("**/api/v1/me", (route) => route.fulfill({ json: { id: "u1", email: "user@example.com", roles: ["user"] } }));
  await page.route("**/api/v1/payments/pay-1/status**", (route) => route.fulfill({ json: { id: "pay-1", status: "pending" } }));
  const target = "/questionario?plano=trimestral&payment_id=pay-1&payment_token=claim-secret";
  await page.goto(`/login?returnTo=${encodeURIComponent(target)}`, { waitUntil: "domcontentloaded" });
  await page.getByLabel("E-mail").fill("user@example.com");
  await page.locator("#login-password").fill("secret123");
