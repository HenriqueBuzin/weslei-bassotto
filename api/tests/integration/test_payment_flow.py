...
async def test_approved_payment_can_only_be_claimed_with_secret(db):
    ...
    payment, secret = await create_payment(
        db, GatewayRegistry([gateway], ["fake"]), plan_slug="trimestral", mode="cash",
        payer_email="card@example.com", card_token="token", payment_method_id="visa",
    )
...
async def test_webhook_is_idempotent_and_renews_once(db):
    ...
    await db.consultancy_submissions.insert_one({
        "_id": submission_id, "customer": {"email": "user@example.com"},
        "plan": {"end_date": "2026-08-01"}, "renewal_count": 0, "renewals": [],
    })
    payment, _ = await create_payment(
        db, GatewayRegistry([gateway], ["fake"]), plan_slug="semestral", mode="subscription",
        payer_email="card@example.com", card_token="token", payment_method_id="master",
        renewal_submission_id=str(submission_id), account_email="user@example.com",
    )
...
async def test_unavailable_gateway_falls_back_to_next_adapter(db):
    payment, _ = await create_payment(
        db, GatewayRegistry([UnavailableGateway(), FakeGateway()], ["unavailable", "fake"]),
        plan_slug="trimestral", mode="cash", payer_email="card@example.com", card_token="token", payment_method_id="visa",
    )
...
async def test_rejected_charge_does_not_try_another_gateway(db):
    with pytest.raises(GatewayRejected):
        await create_payment(
            db, GatewayRegistry([RejectedGateway(), FakeGateway()], ["rejected", "fake"]),
            plan_slug="trimestral", mode="cash", payer_email="card@example.com", card_token="token", payment_method_id="visa",
        )
