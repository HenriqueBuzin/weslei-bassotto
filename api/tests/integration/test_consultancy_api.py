...
async def seed_payment(db, user, secret="claim-secret", status="approved"):
    doc = {
        ...
        "claim_token_hash": token_hash(secret),
        "gateway": "fake",
...
async def test_complete_submission_and_answer_revision_flow(client, db, user_factory, auth_headers):
    ...
    changed = await client.patch(
        ...
        json={"answers": [{"question_id": str(question["_id"]), "value": "Hipertensão controlada"}]},
        headers=headers,
    )
...
async def test_admin_question_crud_and_permissions(client, user_factory, auth_headers):
    ...
    updated = await client.patch(
        f"/api/v1/consultancy/admin/questions/{question_id}",
        json={"label": "Quantos dias treina?"},
        headers=auth_headers(admin),
    )
...
async def test_question_and_submission_edge_cases(client, db, user_factory, auth_headers):
    ...
    assert (
        await client.patch(
            f"/api/v1/consultancy/me/submissions/{submission['_id']}/answers",
            json={"answers": []},
    ...
    assert (
        await client.post(
            f"/api/v1/consultancy/me/submissions/{submission['_id']}/renew",
    ...
    updated = await client.patch(
        f"/api/v1/consultancy/admin/submissions/{submission['_id']}",
        json={"status": "finished", "payment_reference": "ref", "start_date": "2026-02-01", "end_date": None},
    ...
    assert (
        await client.post(
            f"/api/v1/consultancy/admin/submissions/{submission['_id']}/answers/seen", headers=admin_headers
        )
    ).status_code == 200
    assert (
...
async def test_submission_detects_payment_claim_race(db, user_factory, monkeypatch):
    ...
    proxy = SimpleNamespace(
        payments=Payments(),
        consultancy_questions=db.consultancy_questions,
